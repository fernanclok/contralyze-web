...
export default function EditRequisition({ requisition }: { requisition: any }) {
  const [state, setState] = useState<any>(requisition);
  const [items, setItems] = useState<any>(requisition.items || []);
  const [attachments, setAttachments] = useState<File[]>([]);
  const [dragActive, setDragActive] = useState(false);
  const [requestDate, setRequestDate] = useState<any>(new Date(requisition.request_date));
  ...
  const handleEditRequisition = async (
    ...
    const itemsJson = items.map((item: { product_name: string; product_description: string; quantity: number; price: number }) => ({
    ...
    if (!result.errors) {
      ...
      router.push(`/requisitions/details/${requisition.requisition_uid}`);
    }
    setState(result);
  };

  const addItem = () => {
    setItems([...items, { product_name: "", product_description: "", quantity: 0, price: 0 }]);
  };
  ...
  return (
    <>
      <div className="flex items-center mb-6">
        <Link href={`/requisitions/details/${requisition.requisition_uid}`} className="mr-4">
      ...
      <form onSubmit={handleEditRequisition} className="space-y-6">
        {state?.errors?.server && (
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Select name="priority" defaultValue={requisition.priority}>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    ...
                    >
                      ...
                      {requestDate ? format(requestDate, "yyyy-MM-dd") : "Select a date"}
                    </Button>
                  </PopoverTrigger>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {items.map((item: { product_name: string; product_description: string; quantity: number; price: number }, index: number) => (
              <div
                key={index}
